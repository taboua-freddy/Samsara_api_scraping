...
def pandas_to_bq_schema(
    df: pd.DataFrame, time_col: str | None = None
...
def parallelize_execution(tasks: list, func: Any, logger: logging, **kwargs):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        if isinstance(func, str):
            futures = [
                executor.submit(getattr(task, func), **kwargs)
                for task in tasks
                if has_function(task, func)
            ]
        elif callable(func):
            for task in tasks:
...
def extract_date_range(filename) -> list[datetime]:
    ...
    if match:
        ...
        match_range = re.search(
            r"_(\d{4})_(\d{2})_(\d{2})_to_(\d{4})_(\d{2})_(\d{2})", filename
        )
        if match_range:
            end_date_str = (
                match_range.group(4)
...
def parquet_buffer(dataframe: pd.DataFrame) -> io.BytesIO:
    ...
    for col in dataframe.columns:
        if pd.api.types.is_datetime64_any_dtype(dataframe[col]):
            fields.append(
                pa.field(col, pa.timestamp("us"))
            )  # encodé comme TIMESTAMP(MICROS)
        else:
...
def date_to_iso_or_timestamp(
    date: datetime, date_type: Literal["datetime", "date", "timestamp_ms"]
) -> str | float:
        ...
        if len(endpoint_indexes) > 1:
            if params:
                for endpoint_index in endpoint_indexes:
                    current_params = process_params(
                        metadata.loc[endpoint_index, "params"]
                    )
                    if current_params.get("type") and current_params.get(
                        "type"
                    ) == params.get("type"):
