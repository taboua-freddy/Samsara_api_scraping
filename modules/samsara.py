...
from .utils import set_metadata_is_processed
...
class SamsaraClient:
    ...
    def __init__(self, api_token: str, rate_limiter: EndpointRateLimiter, shared_vars_manager: MemoryAccess= None, delta_days: int = 1):
    ...
    def get_all_data(self, endpoint: str, params: dict = None, max_calls_per_second=5) -> list[dict]:
        """
        ...
        while has_next_page:
            ...
            while not success and retry_count < max_retries:
                ...
                try:
                    self.logger.info(f"Demande de données à {url} avec les paramètres {params}")
                    response = requests.get(url, headers=self.headers, params=params)
                    # Gère le succès et les erreurs spécifiques de la requête
                    if response.status_code == 200:
                        data = response.json()
                        self.logger.info(f"Données récupérées avec succès pour {url} et les paramètres {params}")
            ...
            if not success:
                self.logger.error(f"Échec après plusieurs tentatives, arrêt du traitement pour {url} et les paramètres {params}")
                break
