...
class DataFetcher:
    ...
    def split_data(
        ...
        if table_name in self.transformer.tables_to_split:
            results: dict[str, pd.DataFrame] = self.transformer.set_data(
                df, {"table_name": f"{table_name}_split"}
            ).transform()
            for table_name in results:
                if not results[table_name].empty:
                    results[table_name] = self.transformer.set_data(
                        results[table_name], {"table_name": table_name}
                    ).transform()
        else:
